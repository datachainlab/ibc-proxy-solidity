...
library IbcLightclientsProxyV1ClientState {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      if (fieldId == 1) {
        ...
        pointer += _read_ibc_prefix(pointer, bs, r, counters);
      }
      else if (fieldId == 5) {
        pointer += _read_trusted_setup(pointer, bs, r, counters);
      }
    ...
    returns (uint)
  {
    ...
    pointer += IbcCoreCommitmentV1MerklePrefix._encode_nested(r.proxy_prefix, pointer, bs);
    ...
    pointer += IbcCoreCommitmentV1MerklePrefix._encode_nested(r.ibc_prefix, pointer, bs);
    
    if (r.trusted_setup != false) {
    pointer += ProtoBufRuntime._encode_key(
      5,
    ...
    pointer += ProtoBufRuntime._encode_bool(r.trusted_setup, pointer, bs);
    }
...
library IbcLightclientsProxyV1ConsensusState {


  //struct definition
  struct Data {
    GoogleProtobufAny.Data proxy_consensus_state;
  }
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 1) {
        pointer += _read_proxy_consensus_state(pointer, bs, r, counters);
      }
   ...
   * @return The number of bytes decoded
   */
  function _read_proxy_consensus_state(
    uint256 p,
    bytes memory bs,
    Data memory r,
    uint[2] memory counters
    ...
    returns (uint)
  {
    ...
    pointer += GoogleProtobufAny._encode_nested(r.proxy_consensus_state, pointer, bs);
  ...
  function _estimate(
    ...
    uint256 e;
    e += 1 + ProtoBufRuntime._sz_lendelim(GoogleProtobufAny._estimate(r.proxy_consensus_state));
    return e;
  }
  // empty checker
   ...
   */
  function store(Data memory input, Data storage output) internal {
    GoogleProtobufAny.store(input.proxy_consensus_state, output.proxy_consensus_state);
